...
6       continue
  ...
  !****************************************************************
      ...
      !print*,dz1, uprof(indzp), indzp,dz2, uprof(indz),indz,'mjr i'
...
99   continue
  ...
  if (ngrid.ge.0) then
    ...
    xt=xt+real(dxsave*cosfact*real(ldirect),kind=dp) !! bigger change
...
43   continue
  ...
  if (ngrid.ge.0) then
    cosfact=dxconst/cos((yt*dy+ylat0)*pi180)
    xt=xt+real(u*cosfact*real(ldt*ldirect),kind=dp)    !! small change
